...
function changeStyleButton(){
     sayHelloButton.classList.add('styled-button')
}
sayHelloButton.addEventListener('mouseover',changeStyleButton)
...
// CODE HERE
function backButton(){
    sayHelloButton.classList.remove('styled-button')
}
sayHelloButton.addEventListener('mouseout',backButton)
// PROBLEM 4
/*
...
function queryRequest(){
    const query='example'
    axios
    // .get(http://localhost:3000/query-test`) -first scenario
    //  .get('http://localhost:3000/query-test?query1=hello&query2=world') -second
    .get(`http://localhost:3000/query-test/?query=${query}`)
    .then((res)=>{
        repeatText.textContent=res.data
    })
    .catch((err)=>{
